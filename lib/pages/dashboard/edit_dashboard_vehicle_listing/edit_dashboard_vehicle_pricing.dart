...
class EditDashboardVehiclePricing extends StatefulWidget {
  @override
  State createState() => EditDashboardVehiclePricingState();
}

class EditDashboardVehiclePricingState extends  State<EditDashboardVehiclePricing> {
  @override
  void initState() {
    super.initState();
    
    Future.delayed(Duration.zero,() async {
      AppEventsUtils.logEvent("page_viewed", params: {"page_name" : "Edit Dashboard Vehicle Pricing"});
      final Map _receivedData = ModalRoute.of(context)!.settings.arguments as  Map;

      setState(() {
        _carID = _receivedData['ID'];

        _perOneHour = _receivedData['Pricing']['RentalPricing']['PerHour'].toDouble();
        _perOneDay = _receivedData['Pricing']['RentalPricing']['PerDay'].toDouble();
        // _perExtraKm = _receivedData['Pricing']['RentalPricing']['PerExtraKMOverLimit'].toDouble();
        // _perKmDeliveryFee = _receivedData['Pricing']['RentalPricing']['PerKMRentalDeliveryFee'].toDouble();
        _perKmDeliveryFee = 1.0;
        _oneWeekDiscount = _receivedData['Pricing']['RentalPricing']['BookForWeekDiscountPercentage'].toDouble();
        _oneMonthDiscount = _receivedData['Pricing']['RentalPricing']['BookForMonthDiscountPercentage'].toDouble();
        _oneTimeDiscount = _receivedData['Pricing']['RentalPricing']['OneTime20DiscountForFirst3Users'];
        _hasCustomDelivery = _receivedData['Pricing']['RentalPricing']['EnableCustomDelivery'];
                                    ...
                                    children: <Widget>[
                                      Expanded(
                                        child: ElevatedButton(
                                          ...
                                          style: ElevatedButton.styleFrom(
                                            ...
                                            backgroundColor: _hasCustomDelivery ? Color(0xFFFF8F62) : Color(0xFFE0E0E0).withOpacity(0.5),
                                      ...
                                      Expanded(
                                        child: ElevatedButton(
                                          ...
                                          elevation: 0.0,
                                          padding: EdgeInsets.all(16.0),
                                            backgroundColor: _hasCustomDelivery ? Color(0xFFE0E0E0).withOpacity(0.5) : Color(0xFFFF8F62),
                                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8.0)),
...
Future<http.Response> setPricing() async {
  ...
  try {
    res = await http.post(
      Uri.parse(setPricingUrl),
      // setPricingUrl as Uri,
      body: json.encode(
        {
          "CarID": _carID,
          "Pricing": {
