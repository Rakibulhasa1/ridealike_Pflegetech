...
import 'package:ridealike/pages/trips/request_service/host_start_inspect_request.dart';
...
class HostStartTripBloc implements BaseBloc {
  final _startTripDataController =
      BehaviorSubject<HostInspectTripStartResponse>();
  final storage = new FlutterSecureStorage();
  final ImagePicker picker = ImagePicker();

  Function(HostInspectTripStartResponse) get changedStartTripData =>
      _startTripDataController.sink.add;

  Stream<HostInspectTripStartResponse> get startTripData =>
  ...
  pickeImageFromGallery2(
    ...
    if (status) {
      ...
      var imageRes;
      var data;
      resultList = (await ImageUtils.loadAssets()).cast<XFile>();
  ...
  Future<HostInspectTripStartResponse?> startTripInspectionsMethod(
      HostInspectTripStartResponse data) async {
    var resp = await startTripInspections(data);
    if (resp != null && resp.statusCode == 200) {
      HostInspectTripStartResponse inspectTripStartResponse =
          HostInspectTripStartResponse.fromJson(json.decode(resp.body!));
  ...
  checkValidation(HostInspectTripStartResponse data) {
    ...
    if (data.tripStartInspection!.interiorImageIDs![0] == '' &&
        ...
        data.tripStartInspection!.interiorImageIDs![3] == '' &&
        data.tripStartInspection!.interiorImageIDs![4] == '' &&
        data.tripStartInspection!.interiorImageIDs![5] == '') {
      return false;
    }
    if (data.tripStartInspection!.exteriorImageIDs![0] == '' &&
        ...
        data.tripStartInspection!.exteriorImageIDs![3] == '' &&
        data.tripStartInspection!.exteriorImageIDs![4] == '' &&
        data.tripStartInspection!.exteriorImageIDs![5] == '') {
      return false;
    }
    ...
    if (data.tripStartInspection!.fuelLevel!.trim() == '') {
      return false;
    }
