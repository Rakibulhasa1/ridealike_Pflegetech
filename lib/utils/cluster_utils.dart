...
import 'package:ridealike/widgets/car_list_cluster_non_search.dart';
import 'package:ridealike/widgets/car_list_cluster_on_search.dart';
...
class ClusterUtils {
  ...
  static Future<List<Marker>?> updateMarkers([double? updatedZoom]) async {
    ...
    if (_fluster == null || updatedZoom == currentZoom) return null;

    if (updatedZoom != null) {
      currentZoom = updatedZoom;
    }

    updatedMarkers = await Future.wait(_fluster!.clusters(
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return carData == null
            ?
            // non search
            CarListClusterNonSearch(
                stream: stream,
                points: points,
              )
            :
            // search
            CarListClusterOnSearch(
                points: points,
                carData: carData,
                startDate: startDate,
                endDate: endDate,
